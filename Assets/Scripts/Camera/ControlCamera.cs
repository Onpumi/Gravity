...
public class ControlCamera : MonoBehaviour
{
   ...
   void CameraSetPosition(Vector3 target)
   {
      ...
      float interpolationRatio = (float)elapsedFrames / interpolationFramesCount;
	   //transform.position = Vector3.Lerp(transform.position,new_position, _dumping * Time.fixedDeltaTime);
	  // transform.rotation = Quaternion.Lerp(transform.rotation,new_rotate, _dumping * Time.fixedDeltaTime);
      //transform.rotation = Quaternion.Lerp(transform.rotation,new_rotate, interpolationRatio);
      transform.position = Vector3.Lerp(transform.position,new_position, interpolationRatio);
